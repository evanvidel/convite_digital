    ...
    <div className='grid grid-cols-3 gap-5 '>
      {eventos.map((evento) => (
        ...
        >
          ...
          <div className='flex-1 flex flex-col  items-center text-center p-7 gap-5 text-center'>
            <span className='text-lg font-black'>{evento.nome}</span>
            <p className='flex-1 text-sm text-zinc-400'>{evento.descricao}</p>
              <QRCode
              value={JSON.stringify({id: evento.id, senha: evento.senha})} className='w-44 h-44'
              />
